...
export default function MainLayout() {
  ...
  const getNotificationLink = (notification: { referenceType?: string; referenceId?: number }) => {
    ...
    switch (notification.referenceType) {
      case 'transport': return `/transport?id=${notification.referenceId}`;
      case 'location': return `/location?id=${notification.referenceId}`;
      case 'panne': return `/pannes?id=${notification.referenceId}`;
      case 'maintenance': return `/entretien?id=${notification.referenceId}`;
      default: return null;
    }
  };
  ...
  const getRoleLabel = (role: RoleUtilisateur) => ROLE_LABELS[role] || role;
  ...
  const totalNotifCount = (notifCount || 0) + (alertStats?.actives || 0);

  // Sidebar content (shared between desktop and mobile drawer)
  const SidebarContent = ({ collapsed }: { collapsed: boolean }) => (
    <>
      {/* Menu */}
      <nav className="mt-4 px-3 overflow-y-auto flex-1" style={{ maxHeight: 'calc(100vh - 180px)' }}>
        {filteredMenuItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex items-center gap-3 px-3 py-3 mb-1 rounded-lg transition-colors ${isActive
      ...
      <div className="p-4 border-t border-gray-800">
        <div className={`flex items-center ${!collapsed ? 'justify-between' : 'justify-center'}`}>
          {!collapsed && (
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-200">
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 flex flex-col
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 bg-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white font-bold text-lg">ACL Platform</span>
          </div>
          <button
            onClick={() => setMobileOpen(false)}
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 bg-gray-800 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            ...
            {sidebarOpen && <span className="text-white font-bold text-lg truncate">ACL Platform</span>}
          </div>
          <button
      ...
      <main className={[
        ...
        {/* ── Top Header ── */}
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="lg:hidden p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors"
          ...
          {/* Right: Notifications + Theme Toggle + Offline Indicator */}
            ...
            <div className="relative">
              ...
              {showNotifDropdown && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-50 max-h-[80vh] flex flex-col">
                    <div className="p-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center flex-shrink-0">
                        ...
                        <>
                          ...
                          {notifications.slice(0, 5).map((notif) => {
                            ...
                            return (
                              <div
                                key={notif.id}
                                className={`px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-l-4 cursor-pointer ${!notif.lue ? 'border-yellow-500 bg-yellow-50 dark:bg-yellow-900/20' : 'border-transparent'
                                  }`}
                                onClick={() => {
                                  if (!notif.lue) handleMarkNotificationRead(notif.id);
                                  if (link) { setShowNotifDropdown(false); navigate(link); }
                                }}
                              >
                                <div className="flex justify-between items-start">
                                  <p className="text-sm font-medium text-gray-900 dark:text-white">{notif.titre}</p>
                                  {!notif.lue && <span className="w-2 h-2 bg-yellow-500 rounded-full flex-shrink-0 ml-2 mt-1.5" />}
                                </div>
                                {notif.message && <p className="text-xs text-gray-600 dark:text-gray-400 mt-0.5 line-clamp-2">{notif.message}</p>}
                                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{new Date(notif.createdAt).toLocaleString('fr-FR')}</p>
                              </div>
                            );
                          })}
                        </>
                        ...
                        <>
                          ...
                          {recentAlertes.slice(0, 3).map((alerte) => (
                            <Link
                              ...
                              className={`block px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-l-4 ${alerte.niveau === 'CRITICAL' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' :
                            ...
                            >
                              ...
                              {alerte.message && <p className="text-xs text-gray-600 dark:text-gray-400 truncate mt-0.5">{alerte.message}</p>}
                              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{new Date(alerte.createdAt).toLocaleString('fr-FR')}</p>
                            </Link>
                          ))}
                        </>
                      )}
                    ...
                    <Link
                      to="/alertes"
                      onClick={() => setShowNotifDropdown(false)}
                      className="block p-3 text-center text-sm font-medium text-yellow-600 hover:bg-gray-50 dark:hover:bg-gray-700 border-t border-gray-100 dark:border-gray-700 flex-shrink-0"
                    >
